...
def login():
    ...
    if (page_json['status'] == 1):
        # file = open('user.txt', encoding="utf-8", errors="ignore", mode='w')
        # file.write(json.dumps(page_json['data']))
        # file.write(page_json['data']['token'])
        # return page_json['data']  # page_json['data'].token 及userid
    ...
    while (loop != True):
        ...
        params = {
            'per_page': per_page, 'page': page, 'province': global_config.getRaw(
                'config', 'province'), 'city': global_config.getRaw('config', 'city')
        }
        print(threadName, deplay, page, per_page)
        # time.sleep(deplay * 5)
...
def logNone(str):
    with open("logNone.txt", "a", encoding="utf-8") as file:
        file.write(str)


def search():
    print('search')
    loopRequest('a', 1, 1, 10)
    # 创建30个多线程,每页100个数据
